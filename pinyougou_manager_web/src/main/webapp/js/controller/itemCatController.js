...
window.onload=function () {
	...
	var app = new Vue({
		...
		methods:{
				...
				axios.post(url, this.entity).then(function (response) {
					if (response.data.success) {
						...
						// app.findPage(app.pageNo);
							app.findByParentId({id:app.entity.parentId});
					} else {
						//失败时显示失败消息
			...
			findByParentId:function (itemCat) {
				this.entity.parentId =itemCat.id;
				axios.get("/itemCat/findByParentId.do?parentId="+itemCat.id).then(function (resp) {
					app.list = resp.data;
					if (app.count ==1){
						app.entity_2={};
						app.entity_3={};
						...
						app.entity_3={};
						app.entity_2 = itemCat
					} else if (app.count == 3){
						app.entity_3 = itemCat
					}
				})
			}
		},
